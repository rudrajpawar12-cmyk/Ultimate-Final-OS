-- CareerOS base schema (consolidated).
-- Recreates every production table, helper function and RLS policy the app
-- relies on. Idempotent: safe to re-run.
...
-- Owner-scoped tables: full access for the row owner.
do $$
declare
  t text;
begin
...
-- Recruiter-owned tables keyed by recruiters.id
...
-- Applications: candidate own rows + recruiter of the job
...
-- Recruiter read access to their own applicants' candidate data
do $$
declare
  t text;
begin
